...
public class Richards1DSolver {
	...
	@Description("Psi value at the top of the soil column at the current time (top boundary condition)")
	@Unit ("m")
	double psiTop;
	...
	public void solve() {
		...
		for(int i = 0; i < NUM_CONTROL_VOLUMES; i++) {
			if( i == 0 ) {
				...
				mainDiagonal[i] = gridvarsq * (kM + 2*kP);
				upperDiagonal[i] = 0;
				rhss[i] = thetas[i] + gridvar * (kP-kM) + 2 * kP * gridvarsq * psiTop; 
